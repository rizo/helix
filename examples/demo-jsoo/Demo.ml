...
let view_mouse () =
  ...
  let mouse =
    Signal.sample ~on:(Time.tick ~ms:(1000 / 60)) Mouse.position
    |> Signal.map (fun (x, y) ->
...
let view_visibility_simple () =
  ...
  fragment
    [ h2
        [ (* conditional is_visible; *) style [ ("font-family", "monospace") ] ]
        [ text "Html.visible" ]
    ; button
        (* [ on ~prevent:true Event.click (fun _ -> Signal.update not is_visible) ] *)
