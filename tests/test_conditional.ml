...
let test_simple () =
  let open Html in
  div []
    [ conditional ~on:(Signal.make true) (div [] [ text "present" ])
    ; conditional ~on:(Signal.make false) (footer [] [ text "missing" ])
    ]
...
let test_toggle_simple () =
  ...
  div []
    ...
    ; ul [] [ li [] [ conditional ~on:is_present (span [] [ text "HELLO" ]) ] ]
    ]
...
let test_toggle_siblings () =
  ...
  div []
    ...
    ; button
        ...
        [ text "Toggle BOTH" ]
    ; ul []
        [ li [] [ conditional ~on:hello (span [] [ text "HELLO 1" ]) ]
        ; li [] [ conditional ~on:hello (span [] [ text "HELLO 2" ]) ]
        ]
    ; ul []
        [ li [] [ span [] [ text "before 1" ] ]
        ; li [] [ span [] [ text "before 2" ] ]
        ; li [] [ conditional ~on:hello (span [] [ text "HELLO" ]) ]
    ...
    ; ul []
        [ li [] [ conditional ~on:hello (span [] [ text "HELLO" ]) ]
        ; li [] [ conditional ~on:bye (span [] [ text "BYE" ]) ]
        ]
    ; ul []
        [ li [] [ span [] [ text "before 1" ] ]
        ; li [] [ conditional ~on:bye (span [] [ text "BYE 1" ]) ]
        ; li [] [ conditional ~on:hello (span [] [ text "HELLO 1" ]) ]
        ; li [] [ conditional ~on:hello (span [] [ text "HELLO 2" ]) ]
        ; li [] [ conditional ~on:bye (span [] [ text "BYE 2" ]) ]
        ; li [] [ span [] [ text "after 1" ] ]
        ]
    ]
