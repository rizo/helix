...
module Date : sig
  ...
  let of_string str =
    ...
    | [ yyyy; mm; dd ] when String.length yyyy = 4 && String.length dd = 2 && String.length mm = 2
      -> Some str
    | _ -> None
...
module Device_metrics = struct
  module Links = struct
    let date =
      Router.var ~of_string:Date.of_string ~to_string:Date.to_string ~equal:Date.equal "date"
    ...
    div []
          ...
          [ (*show (fun path -> code [] (String.concat "/" path)) (Router.path router);*)
            ul []
              ...
              ; li []
                  ...
                  ; input
                      [ type' "date"
                      ; bind (fun date -> value (Date.to_string date)) start_date
                      ; on Event.input (fun ev ->
                            let date =
                              Event.target ev |> Node.get_value |> Date.of_string |> Option.get
                            in
      ...
      ; div
          ...
          [ Router.dispatch ~label:"metrics" router
              [ Router.route Links.index (fun () -> Html.text "PICK METRIC ABOVE")
...
let view router =
  ...
  div []
    ...
    ; ul []
        ...
        ; li []
            [ a
                [ Router.link router ~active:(style_list [ ("font-weight", "bold") ]) Links.account
                ]
                [ text "#/account" ]
            ]
        ; li []
        ...
        ; li []
            [ a
                [ Router.link router
                    ~active:(style_list [ ("font-weight", "bold") ])
                    Links.devices "dev_1" Device.Links.metrics Device_metrics.Links.index
                ]
        ...
        ; li []
            [ a
                [ Router.link router
                    ~active:(style_list [ ("font-weight", "bold") ])
                    Links.devices "dev_2" Device.Links.metrics Device_metrics.Links.index
                ]
    ...
    ; Router.dispatch router ~label:"main" ~default:(text "NOT FOUND")
        ...
        ; Router.route Links.device_schema_edit (fun dev_id () ->
              let$ dev_id in
              Html.text ("DEVICE SCHEMA EDIT: " ^ dev_id)
          )
        ]
    ]
